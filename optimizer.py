...
h1_th=Dense(100,activation='tanh',kernel_regularizer=l2(0))(r_i_th)
...
model_sel.compile(optimizer='sgd', loss=MSE_observed_ratings,  metrics=['mae'])

r_i_sp=Input(shape=(num_users,))
h1_sp=Dense(100,activation='tanh',kernel_regularizer=l2(0))(r_i_sp)
r_hat_i_sp=Dense(num_users,activation='linear')(h1_sp)
model_sp=Model(inputs=r_i_sp, outputs=r_hat_i_sp)
model_sp.compile(optimizer='RMSprop', loss=MSE_observed_ratings,  metrics=['mae'])
...
x_1=np.argmin(val_mse)
x_2 = np.argmin(val_mse_sel)
x_3 = np.argmin(val_mse_sp)


y_1=np.min(val_mse)
y_2 = np.min(val_mse_sel)
y_3 = np.min(val_mse_sp)
...
ax.plot(xax, val_mse_sel,  linewidth=1, color='r', marker='o',markerfacecolor='b', label='sgd', linestyle='-', markersize=1)
ax.plot(xax, val_mse,  linewidth=1, color='k', marker='o', markerfacecolor='b',
            label='adam', linestyle='-',markersize=1)
ax.plot(xax, val_mse_sp,  linewidth=1, color='g', marker='o', markerfacecolor='b',
            label='RMSprop', linestyle='-',markersize=1)

plt.plot((0, y_1), (num_epochs, y_1), 'k-')
plt.plot((0, y_2), (num_epochs, y_2), 'k-')
plt.plot((0, y_3), (num_epochs, y_3), 'k-')
plt.plot((x_1, 0), (x_1, 2), 'k-')
plt.plot((x_2, 0), (x_2, 2), 'k-')
plt.plot((x_3, 0), (x_3, 2), 'k-')



ax.legend()
plt.show(block=False)
plt.savefig('optimizer-250epochs.png')

np.savez('optimizer-250',adam=val_mse,sgd=val_mse_sel,RMSprop=val_mse_sp,xax=xax)
