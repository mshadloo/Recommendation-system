...
h1=Dense(100,activation='tanh',kernel_regularizer=l2(0))(r_i)
...
h1 = Dense(100, activation='tanh', kernel_regularizer=l2(0))(r_i)
r_hat_i = Dense(num_users, activation='linear')(h1)
model50 = Model(inputs=r_i, outputs=r_hat_i)
model50.compile(optimizer='adam', loss=MSE_observed_ratings, metrics=['mae'])
...
train_mse = np.zeros((num_epochs, 1))
train_score = np.zeros((num_epochs, 1))
val_mae_50 = np.zeros((num_epochs, 1))
val_mse_50 = np.zeros((num_epochs, 1))
val_score = np.zeros((num_epochs, 1))
...
for epoch in range(num_epochs):
    ...
    val_mse_50[epoch], val_mae_50[epoch] = evaluate(model50, x_val_dict, y_val_dict)
    ...
    for b in range(num_batches):
        ...
        for i in range(b_size):
            ...
            batch[i, x_train_dict[item_ind][0]] = x_train_dict[item_ind][1]
        # print("batch "+ str(b) +" is made")
        model50.fit(batch, batch, epochs=1, verbose=0)  # x_train=y_train

r_i = Input(shape=(num_users,))
h1 = Dense(100, activation='tanh', kernel_regularizer=l2(0))(r_i)
r_hat_i = Dense(num_users, activation='linear')(h1)
model200 = Model(inputs=r_i, outputs=r_hat_i)
model200.compile(optimizer='adam', loss=MSE_observed_ratings, metrics=['mae'])
...
val_mae_200 = np.zeros((num_epochs, 1))
val_mse_200 = np.zeros((num_epochs, 1))
val_score = np.zeros((num_epochs, 1))
...
for epoch in range(num_epochs):
    ...
    val_mse_200[epoch], val_mae_200[epoch] = evaluate(model200, x_val_dict, y_val_dict)
    ...
    for b in range(num_batches):
        ...
        for i in range(b_size):
            ...
            batch[i, x_train_dict[item_ind][0]] = x_train_dict[item_ind][1]
        # print("batch "+ str(b) +" is made")
        model200.fit(batch, batch, epochs=1, verbose=0)  # x_train=y_train

xax = np.arange(0,num_epochs,1)
np.savez('batchsize',adam=val_mse,sgd=val_mse_50,RMSprop=val_mse_200,xax=xax)
fig = plt.figure()
ax = fig.gca()
...
ax.plot(xax, val_mse_50,  linewidth=1, color='r', marker='o',markerfacecolor='b', label='50', linestyle='-', markersize=1)
ax.plot(xax, val_mse,  linewidth=1, color='k', marker='o', markerfacecolor='b',
            label='100', linestyle='-',markersize=1)
ax.plot(xax, val_mse_200,  linewidth=1, color='g', marker='o', markerfacecolor='b',
            label='200', linestyle='-',markersize=1)
ax.legend()
